...
export default function Referrals({ notify }) {
  const [referrals, setReferrals] = useState([]);
  const [loading, setLoading] = useState(true);
  const [refSearch, setRefSearch] = useState('');

  const fetchReferrals = async (search = '') => {
    try {
      setLoading(true);
      const url = search ? `/admin/referrals?search=${search}` : '/admin/referrals';
      const res = await API.get(url);
      setReferrals(res.data);
    } catch (e) {
      console.error(e);
      if (notify) notify('Failed to fetch referrals', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      {loading ? (
        <div style={{padding:20, color:'var(--dim)'}}>Loading referrals...</div>
      ) : (
